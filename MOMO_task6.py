...
if __name__ == "__main__":
    ...
    parser.add_argument("--opt", default='4lde', choices=['qed', 'logP', 'qedplogp','qeddrd', '4lde'])
    ...
    orismi_all = pd.read_csv('./data/oripops/QMO_4dle_mol50_optsmiles.csv').values
    ...
    for num in range(len(a)):
        mm1 = a[num]
        mm2 = b[num]
        smi_iter_all = [['SMILES', 'mol_id','iter', 'qed', 'sim', '4lde']]
        ...
        #调试
                ...
                while iter <= nIter:
                    ...
                    for i in range(len(smis)):
                        if smis[i] not in unique_smiles_iter:
                            ...
                            tuple = [smis[i], nn, iter, fits[i][0], fits[i][1], -fits[i][2]]
                    ...
                    if len(fits) == 0:
                        ...
                        dominated_hypervolume = pg.hypervolume(np.array([[-1.0,-1,-0.1] * np.array(fit) for fit in fits if
                                                                         (np.array(fit) > [0,0,0]).all()])).compute(
                            np.zeros(len(fits[0])))
                        pure_div = PD_cal(np.array([[1,1,0.1] * np.array(fit) for fit in fits if (np.array(fit) > [0,0,0]).all()]))
            ...
            for i in range(len(endsmiles)):
                if endsmiles[i] not in unique_smiles:  # and endfits[i][1] >= 0.4 and endfits[i][0] >= 0.9
                    ...
                    tuple = (endsmiles[i], nn, paretoFits[i][0], paretoFits[i][1],-paretoFits[i][2])
            ...
            np.savetxt('./results/docking/MOMO_4lde_mol'+str(a[0])+str(b[-1])+'_HV.txt', HV, fmt='%s')
            np.savetxt('./results/docking/MOMO_4lde_mol' + str(a[0]) + str(b[-1]) + '_PD.txt', PD, fmt='%s')
            df_to_save_A_to_B = DataFrame(smi_iter_last, columns=col)
            df_to_save_A_to_B.to_csv('./results/docking/MOMO_4lde_mol'+str(a[0])+str(b[-1])+'_endsmiles.csv', index=False)
            ...
            np.savetxt('./results/docking/MOMO_4lde_mol' + str(mm1)+str(mm2) + '_iter.txt', smi_iter_all, fmt='%s')
            np.savetxt('./results/docking/MOMO_4lde_mol'+str(a[0])+str(b[-1])+'.txt', result, fmt='%s')
            np.savetxt('./results/docking/MOMO_4lde_mol'+str(a[0])+str(b[-1])+'_runtime.txt', runtime, fmt='%s')
    # 从原始种群和子种群中筛选nPop个
'''
'''
'''
