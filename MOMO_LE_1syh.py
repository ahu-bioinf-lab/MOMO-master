...
if __name__ == "__main__":
    ...
    parser.add_argument("--opt", default='le-1syh', choices=['qed', 'logP', 'qedplogp','qeddrd', '4lde','le-1syh'])
    ...
    if args.opt == 'qed':
        ...
        tres = [0.8, 0.3, 10]
        col = ['SMILES', 'mol_id', 'qed', 'sim', '4lde']
    elif args.opt == 'le-1syh':
        ff = ff_le_1syh
        fitness = fitness_le_1syh
        tres = [0.3, 0.38]
        col = ['SMILES', 'mol_id', 'sim', 'LE-1SYH','1syh','HA']
    ...
    model = CDDDModel()
    # canonicalize
    data = pd.read_csv('./data/test_1SYH.csv').values
    #orismi_all = pd.read_csv('./data/oripops/QMO_1SYH_optsmiles.csv').values
    ...
    # 参数设置
    pern = 20     # 扰动隐向量的个数，扰动多个然后排序选npop个
    sita = 0.5      #扰动程度的参数
    nPop = 5      #种群个体数
    pc = 1          #交叉率
    pm = 0.5        #变异率
    ...
    #a = list(range(3, 34, 10))  # 180
    #b = list(range(13, 44, 10))  # 200
    a=[0]
    b=[1]
    for num in range(len(a)):
        mm1 = a[num]
        mm2 = b[num]
        smi_iter_all = [['SMILES', 'mol_id','iter', 'sim', 'le-1syh','1syh','HA']]
        ...
        #调试
            ...
            while r <= restart:
                ...
                #print('disfits',disfits)
                ranks = nonDominationSort(dispop, disfits[:, 0:2])  # 非支配排序，对潜向量还是分子？？
                distances = crowdingDistanceSort(dispop, disfits[:, 0:2], ranks)  # 拥挤度
                #if len(fits_pops)==0:
                ...
                while iter <= nIter:
                    ...
                    if len(fits) == 0:
                        ...
                        fits_cal = fits[:, 0:2].astype(np.float32)
                        #print('fits_cal', fits_cal)
                        dominated_hypervolume = pg.hypervolume(np.array(
                            [-1.0 * np.array(fit) for
                             fit in fits_cal if
                             (np.array(fit) >= [0, 0]).all()])).compute(np.array([0, 0]))
                        pure_div = PD_cal(np.array([fit for fit in fits_cal if (np.array(fit) >= [0, 0]).all()]))
                    hv.append((dominated_hypervolume))
                    pure_d.append(pure_div)
                rr = []  # 储存endfits是否满足>=0.9,>=0.4，True=1，False=0
                for i in range(len(smis)):
                    rr.append((np.array(fits_cal[:][i]) >= tres).all())
                if 1 in rr:
                    r = restart + 1
                    SR = SR + 1
                else:
            ...
            #print(endsmiles)
            ranks = nonDominationSort(endsmiles, fits[:, 0:2])  # 非支配排序
            distances = crowdingDistanceSort(endsmiles, fits[:, 0:2], ranks)  # 拥挤度
            ...
            np.savetxt('./results/LE-1SYH/MOMO_LE_mol'+str(a[0])+str(b[-1])+'_HV.txt', HV, fmt='%s')
            np.savetxt('./results/LE-1SYH/MOMO_LE_mol' + str(a[0]) + str(b[-1]) + '_PD.txt', PD, fmt='%s')
            df_to_save_A_to_B = DataFrame(smi_iter_last, columns=col)
            df_to_save_A_to_B.to_csv('./results/LE-1SYH/MOMO_LE_mol'+str(a[0])+str(b[-1])+'_endsmiles.csv', index=False)
            #df_to_save_A_to_B = DataFrame(smi_iter_all, columns=['SMILES', 'mol_id','iter', 'qed', 'sim', '4lde'])
            #df_to_save_A_to_B.to_csv('./results/docking/MOMO_4lde_mol'+str(mm1)+str(mm2)+'_iter.csv', index=False)
            np.savetxt('./results/LE-1SYH/MOMO_LE_mol' + str(mm1)+str(mm2) + '_iter.txt', smi_iter_all, fmt='%s')
            np.savetxt('./results/LE-1SYH/MOMO_LE_mol'+str(a[0])+str(b[-1])+'.txt', result, fmt='%s')
            np.savetxt('./results/LE-1SYH/MOMO_LE_mol'+str(a[0])+str(b[-1])+'_runtime.txt', runtime, fmt='%s')
    # 从原始种群和子种群中筛选nPop个
'''
'''
'''
