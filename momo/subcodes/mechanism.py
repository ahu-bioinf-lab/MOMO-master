...
class Fitness:
    ...
    def __init__(self, config_fitness) -> None:
        self.calls = 0
        self.task = getattr(self, config_fitness)
    ...
    def batch_score(self, molecules):
        fitnesses_list = []
        for objective in self.task():
            fitnesses_list.append(objective.score_list([molecule for molecule in molecules]))#.smiles
        fitness = list(zip(*fitnesses_list))
        return fitness

    # add sim
    def batch_score_add_sim(self, molecules,lead_mol):
        fitnesses_list = []
        for objective in self.task():
            fitnesses_list.append(objective.score_list([molecule for molecule in molecules]))#.smiles
        sim_list=[tanimoto_sim(mo,lead_mol) for mo in molecules]
        fitnesses_list.append(sim_list)
        fitness = list(zip(*fitnesses_list))
        return fitness
...
# sim
def tanimoto_sim(molecule,lead_mol):


    mol1=Chem.MolFromSmiles(molecule)
    mol2=Chem.MolFromSmiles(lead_mol)
